...
| 信号 | 方向 | 描述 |
|------|------|------|
| `pcie_refclk_p` / `pcie_refclk_n` | 输入 | PCIe 参考时钟差分对（典型 100 MHz） |
| `dut_clk` | 输出 | DUT 参考时钟（从 PCIe 参考时钟派生） |

PCIe 参考时钟作为差分信号（正负两路）传递，遵循标准 PCIe 约定。
...
MMIO（存储器映射 I/O）接口是与 `chip_mem` 分离的另一个 AXI4 通道，提供对系统中控制寄存器和外设的访问。此接口连接到 Buckyball 的 MMIO 控制器，允许驱动程序代码与硬件子系统交互。

**写入通道：**

| 信号 | 宽度 | 方向 | 描述 |
|------|------|------|------|
| `mmio_awid` | 4 | 输入 | 写入地址 ID |
| `mmio_awaddr` | 32 | 输入 | 写入地址（32 位可寻址） |
| `mmio_awlen` | 8 | 输入 | 突发长度 |
| `mmio_awsize` | 3 | 输入 | 突发大小编码 |
| `mmio_awburst` | 2 | 输入 | 突发类型 |
| `mmio_awvalid` | 1 | 输入 | 写入地址有效 |
| `mmio_awready` | 1 | 输出 | 写入地址就绪 |
| `mmio_wdata` | 64 | 输入 | 写入数据（8 字节） |
| `mmio_wstrb` | 8 | 输入 | 写入选通（字节使能） |
| `mmio_wlast` | 1 | 输入 | 突发中的最后一拍 |
| `mmio_wvalid` | 1 | 输入 | 写入数据有效 |
| `mmio_wready` | 1 | 输出 | 写入数据就绪 |
| `mmio_bid` | 4 | 输出 | 写入响应 ID |
| `mmio_bresp` | 2 | 输出 | 写入响应（0=OK，1=EXOK，2=SLVERR，3=DECERR） |
| `mmio_bvalid` | 1 | 输出 | 写入响应有效 |
| `mmio_bready` | 1 | 输入 | 写入响应就绪 |

**读取通道：**

| 信号 | 宽度 | 方向 | 描述 |
|------|------|------|------|
| `mmio_arid` | 4 | 输入 | 读取地址 ID |
| `mmio_araddr` | 32 | 输入 | 读取地址 |
| `mmio_arlen` | 8 | 输入 | 突发长度 |
| `mmio_arsize` | 3 | 输入 | 突发大小编码 |
| `mmio_arburst` | 2 | 输入 | 突发类型 |
| `mmio_arvalid` | 1 | 输入 | 读取地址有效 |
| `mmio_arready` | 1 | 输出 | 读取地址就绪 |
| `mmio_rid` | 4 | 输出 | 读取数据 ID |
| `mmio_rdata` | 64 | 输出 | 读取数据 |
| `mmio_rresp` | 2 | 输出 | 读取响应 |
| `mmio_rlast` | 1 | 输出 | 突发中的最后一拍 |
| `mmio_rvalid` | 1 | 输出 | 读取数据有效 |
| `mmio_rready` | 1 | 输入 | 读取数据就绪 |

MMIO 接口使用与 `chip_mem` 相同的 AXI4 协议，但针对外设地址空间。寄存器访问模式和响应时序是特定于实现的；参考 Pegasus Shell 文档了解外设映射。
...
Pegasus 框架对 DUT 操作使用 250 MHz 的参考时钟频率。这是 DUT（`dut_clk`）运行的频率，从 PCIe 参考时钟输入派生。配置 DRAMSim2 或时序敏感的工作负载时，确保 CPU 时钟频率参数与此 250 MHz 规范相符。
